...
sos=butter(2,np.array([150,650])/fs*2,output='sos', btype='bandpass')# membrane filtering
...
if save_flag==True:
    ...
    for nl,level in enumerate(level_sig):
        ...
        for mdt,dts in enumerate(itd_sig):
            ...
            patterns=IC.IC_Filterbank_fun(1.*stereo,fs, itd_list,_flow=-1)
            patterns=np.transpose(patterns, (1,0,2))
                

            savestruct[key_str]={'patterns':patterns}

    with open('pkl/fP.pkl','wb') as fd:
        pickle.dump(savestruct,fd)
else:
    with open('pkl/fP.pkl','rb') as fd:
...
fig.savefig('fig7P.pdf')
figNo.savefig('fig7PNo.pdf')
plt.show(block=0)
